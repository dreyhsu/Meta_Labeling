...
# import data from Data (finlab course)
# update raw data first before training
parser = ArgumentParser()
parser.add_argument("--n", help="stock_id", type=int)
...
# training_data 不須要全部的data，可以使用近期的資料作訓練即可
...
def Select_by_sid(sid, close, open_, high , low, vol, rev, com_rev, d_yield, pb, benchmark):
    t_final = 5
    ...
    # data['upperband1'] = data['upperband'].shift(1)
    # data['lowerband1'] = data['lowerband'].shift(1)
    # 測試隔天上漲或下跌是否會影響勝率
    # data['next_close'] = data['close'].shift(-1)
    # data['diff'] = (data['next_close'] - data['close'])/data['close']
    # bband mean reversed
    # long_signals = (data['close'] >= data['MA5']) & (data['low1'] <= data['upperband']) & (data['close'] > data['open'])
    # short_signals = (data['close'] <= data['MA5']) & (data['high1'] >= data['lowerband']) & (data['open'] > data['close'])
    con1 = (data['close'] >= data['lowerband']) & (data['low1'] <= data['lowerband']) & (data['close'] > data['close1'])
    con2 = (data['close'] <= data['upperband']) & (data['high1'] >= data['upperband']) & (data['close1'] > data['close'])
    con3 = (data['high'] >= data['upperband']) & (data['high1'] >= data['upperband']) & (data['close'] > data['close1'])
    con4 = (data['low'] <= data['lowerband']) & (data['low1'] <= data['lowerband']) & (data['close1'] > data['close'])
    con5 = (data['close'] >= data['_MA20']) & (data['low1'] <= data['_MA20']) & (data['close'] > data['close1'])
    con6 = (data['close'] <= data['_MA20']) & (data['high1'] >= data['_MA20']) & (data['close1'] > data['close'])
    # long_signals = data['low'] <= data['lowerband']
    # short_signals = data['high'] >= data['upperband']
    # KD mean reversed
    # long_signals = (data['K'] <= 50) & (data['close'] >= data['MA5']) & (data['close1'] <= data['MA5'])
    # short_signals = (data['K'] >= 50) & (data['close'] <= data['MA5']) & (data['close1'] >= data['MA5'])

    data.loc[con1, 'side'] = 1
    data.loc[con2, 'side'] = 2
    data.loc[con3, 'side'] = 3
    data.loc[con4, 'side'] = 4
    data.loc[con5, 'side'] = 5
    data.loc[con6, 'side'] = 6
    ...
    if args.n:
        ...
        X = data.copy()
        # X.drop(['upperband', 'lowerband', 'middleband', 'open', 'high', 'low','rev','volume', 'cs1', 'close','MA5','MA60',
        #  'vol1', 'fast_mavg', 'slow_mavg'], axis=1, inplace=True)
        feature_list = ['date', 'side', 'sma', 'out', 'long_ret', 'short_ret'] + feature_list 
...
def main():
    if args.n:
        X = Select_by_sid(str(args.n), close, open_, high , low, vol, rev, com_rev, d_yield, pb, benchmark)
        if args.ls == 'long':
            X.to_csv(f'training_data/{str(args.n)}_long.csv', index=False)
        else:
            X.to_csv(f'training_data/{str(args.n)}_short.csv', index=False)
    else:
        sid_list = pd.read_csv('sid.csv')
        tar_list = list(sid_list['sid'].unique())
        # tar_list = ['2330']
        # X = Select_by_sid('2330', close, open_, high , low, vol, rev, com_rev, d_yield, pb, benchmark)
        for i, item in enumerate(tqdm(tar_list)):
            sid = str(item)
            if i == 0:
                X = Select_by_sid(sid, close, open_, high , low, vol, rev, com_rev, d_yield, pb, benchmark)
            else:
                try:
                    X = pd.concat([X, Select_by_sid(sid, close, open_, high , low, vol, rev, com_rev, d_yield, pb, benchmark)])
                except KeyError:
                    continue
                except:
                    continue
        ...
        if args.ls == 'long':
            X.to_csv('training_data/bband_training_data_long_31_test.csv', index=False)
        else:
            X.to_csv('training_data/bband_training_data_short0520.csv', index=False)
